...
exports.updateRestaurant = asyncHandler(async (req, res, next) => {
  ...
  if (restaurant.owner.toString() !== req.user.id && req.user.role) {
    return next(
      new ErrorResponse(
...
exports.deleteRestaurant = asyncHandler(async (req, res, next) => {
  ...
  if (restaurant.owner.toString() !== req.user.id && req.user.role) {
    return next(
      new ErrorResponse(
        `User ${req.params.id} is not authorized to delete`,
        403
      )
    );
  }
...
exports.restaurantUploadPhoto = asyncHandler(async (req, res, next) => {
  ...
  if (file.size > process.env.MAX_FILE_UPLOAD) {
    return next(
      new ErrorResponse(
        `The image need to be less than ${process.env.MAX_FILE_UPLOAD}`,
        400
      )
    );
  }
